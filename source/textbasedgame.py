...
# Initialize game and ui objects
game = Game()
game.ui = UI()

player = {'currentRoom': 'arthelmTown', 'hp': 100, 'attack': 10}
game.mode = 'normal'
...
def move(direction):
    ...
    if game.gameMaps['arthelm']['rooms'][currentRoom][direction]:
        player['currentRoom'] = game.gameMaps['arthelm']['rooms'][currentRoom][direction]['room']
    else:
...
def calculate_room_positions():
    startPos = (0, 0)
    roomPositions = {player['currentRoom']: startPos}
    queue = [(player['currentRoom'], startPos)]
    directions = {
        'n': 'north', 'ne': 'northeast', 'e': 'east', 'se': 'southeast', 
        's': 'south', 'sw': 'southwest', 'w': 'west', 'nw': 'northwest'
    }
...
def draw_minimap():
    ...
    playerPos = roomPositions[player['currentRoom']]
    ...
    currentRoomPos = roomPositions[player['currentRoom']]
    game.ui.minimap.create_text(currentRoomPos[0], currentRoomPos[1]-20, text=game.gameMaps['arthelm']['rooms'][player['currentRoom']]['name'], fill='white', font=('Helvetica', 12))  # Increased text size
...
game.ui.nodeNameLabel = tk.Label(game.ui.middleColumn, text=game.gameMaps['arthelm']['rooms'][player['currentRoom']]['name'], bg='#666666', fg='#FFFFFF', font=('Helvetica', 24))
...
game.ui.talkButton = tk.Button(game.ui.actionButtonFrame, text='Talk', **button_style)
...
send_message(
    game.gameMaps['arthelm']['rooms'][player['currentRoom']]['name'],
    game.gameMaps['arthelm']['rooms'][player['currentRoom']]['description']
)
